        ...
        public function prepAndBind($query, $args){
            ...
            try{
                if($db->execute($args)){
                    ...
                    throw new ErrorHandler("Error: Unable to process request. Query: " . $query);
                }

            } catch (ErrorHandler $e){

                $_toFile = (strpos($query, $GLOBALS['settings']['ErrorHandler']['Database']['table']) !== false) ?
                            true : false;

                $e->write_to_errors($_toFile);
        ...
        public function fetchAll($stmt, $type = null){
            ...
            try{
                if($stmt != null) {
                    ...
                    throw new ErrorHandler("Error: Unable to load information from PDOStatement.");
                }
            } catch (ErrorHandler $e){
                $e->write_to_errors();
                return null;
